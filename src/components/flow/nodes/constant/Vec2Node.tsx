...
export default function Vec2Node({ id, data }: { id: string; data: Vec2NodeData }) {
  ...
  const [localValue, setLocalValue] = useState({ x: data.x ?? 0, y: data.y ?? 0 });

  const outputs: PortDef[] = [{ label: 'out', shaderType: ShaderType.Vec2 }];

  // Update local value when data changes
  useEffect(() => {
    setLocalValue({ x: data.x ?? 0, y: data.y ?? 0 });
  }, [data.x, data.y]);
  ...
  const saveAndClose = useCallback(() => {
    setNodes((nds: any) =>
      nds.map((node: any) =>
        node.id === id
          ? { ...node, data: { ...node.data, x: localValue.x, y: localValue.y } }
          : node
      )
    );
    setShowEditor(false);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: 'x' | 'y') => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (field === 'x') {
