...
export default function Vec3Node({ id, data }: { id: string; data: Vec3NodeData }) {
  ...
  const [localValue, setLocalValue] = useState({ x: data.x ?? 0, y: data.y ?? 0, z: data.z ?? 0 });

  const outputs: PortDef[] = [{ label: 'out', shaderType: ShaderType.Vec3 }];

  // Update local value when data changes
  useEffect(() => {
    setLocalValue({ x: data.x ?? 0, y: data.y ?? 0, z: data.z ?? 0 });
  ...
  const saveAndClose = useCallback(() => {
    setNodes((nds: any) =>
      nds.map((node: any) =>
        node.id === id
          ? { ...node, data: { ...node.data, x: localValue.x, y: localValue.y, z: localValue.z } }
          : node
      )
    );
    setShowEditor(false);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: 'x' | 'y' | 'z') => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (field === 'x') {
  ...
  return (
    <NodeWrapper title={data.label ?? 'Vec3'} outputs={outputs} icon={<NodeIcon type="vec3" />}>
      <div className="relative">
        <div
          ref={valueRef}
          className="grid grid-cols-3 gap-x-2 gap-y-1 text-sm font-mono cursor-pointer hover:opacity-80 select-none"
